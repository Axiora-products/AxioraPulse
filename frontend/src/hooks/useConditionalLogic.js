  ...
  function evaluate(rule, answers) {
    ...
    switch (operator) {
      ...
      case 'not_includes':
        ...
        return !ansStr.includes(valStr);
      case 'gte':
        return Number(ans) >= Number(value);
      case 'lte':
        return Number(ans) <= Number(value);
      case 'first_is':
...
export function evaluateCondition(rule, answers) {
  ...
  switch (operator) {
    case 'equals':
      return n(ans) === n(value);
    case 'not_equals':
      return n(ans) !== n(value);
    case 'contains':
      return n(ans).includes(n(value));
    case 'not_contains':
      return !n(ans).includes(n(value));
    case 'includes':
      return Array.isArray(ans) ? ans.map(n).includes(n(value)) : n(ans).includes(n(value));
    case 'not_includes':
      return Array.isArray(ans) ? !ans.map(n).includes(n(value)) : !n(ans).includes(n(value));
    case 'gte':
      return Number(ans) >= Number(value);
    case 'lte':
      return Number(ans) <= Number(value);
    case 'first_is':
      return Array.isArray(ans) && n(ans[0]) === n(value);
    default:
      return true;
  }
}
