  ...
  return useMemo(() => {
    ...
    if (total > 0 && qs.length > 0) {
      rsW.forEach((r) => {
        const answered = new Set(
          ansW.filter((a) => a.response_id === r.id).map((a) => a.question_id)
        );
        const pct = answered.size / qs.length;
        if (pct >= 0.25) milestones.pct25++;
        if (pct >= 0.5) milestones.pct50++;
    ...
    if (npsQ) {
      ...
      if (scores.length) {
        ...
        nps = {
          score,
          label,
          breakdown: { promoters, passives, detractors, total: scores.length },
        };
      }
    }
    ...
    const dropOffFunnel = qs.map((q, i) => {
      ...
      const responseIdsReached = new Set(
        ansW.filter((a) => qIdsFromHere.includes(a.question_id)).map((a) => a.response_id)
      );
      ...
      const prevReached =
        i === 0
          ...
          : (() => {
              ...
              return new Set(
                ansW.filter((a) => prevIds.includes(a.question_id)).map((a) => a.response_id)
              ).size;
            })();
    ...
    const timingHeatmap = qs.map((q) => {
      ...
      const avgSecs = vals.length ? Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
      const label =
        avgSecs < 5 ? 'Fast' : avgSecs < 15 ? 'Normal' : avgSecs < 30 ? 'Slow' : 'Very slow';
      return {
        questionId: q.id,
        questionText: q.question_text,
        avgSecs,
    ...
    const trendMap = {};
    days.forEach((d) => {
      trendMap[d] = { started: 0, completed: 0 };
    });
    rsW.forEach((r) => {
...
function computeQuestionData(q, ans) {
  ...
  if (['single_choice', 'dropdown', 'yes_no'].includes(q.question_type)) {
    ...
    const labels = Object.keys(c).map(
      (k) => (q.options || []).find((o) => o.value === k)?.label || k
    );
  ...
  if (q.question_type === 'slider') {
    ...
    const labels = counts.map(
      (_, i) => `${Math.round(qMin + i * step)}–${Math.round(qMin + (i + 1) * step)}`
    );
