          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
              ...
              <button id="invite-button"
                onClick={() => { if (hasNoDomains) return toast.error('Configure approved email domains in Settings before inviting.'); if (teamFull) return toast.error('Team size limit reached for your plan.'); setShowModal(true); }}
                disabled={hasNoDomains || teamFull}
                style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: (hasNoDomains || teamFull) ? 'rgba(22,15,8,0.2)' : 'var(--espresso)', color: 'var(--cream)', fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', padding: '14px 28px', borderRadius: 999, border: 'none', cursor: (hasNoDomains || teamFull) ? 'not-allowed' : 'pointer', transition: 'background 0.25s ease' }}
                onMouseEnter={e => { if (!hasNoDomains && !teamFull) e.currentTarget.style.background = 'var(--coral)'; }}
                onMouseLeave={e => { if (!hasNoDomains && !teamFull) e.currentTarget.style.background = 'var(--espresso)'; }}>
                + Invite member
              </button>
            </div>
          );
