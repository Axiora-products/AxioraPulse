...
const tip = {
  ...
  bodyFont: { family: 'Fraunces', size: 12 },
  titleColor: 'rgba(253,245,232,0.55)',
  bodyColor: '#FDF5E8',
};
const barOpts = {
  responsive: true,
  ...
  scales: {
    x: {
      grid: { display: false },
      ticks: { font: { family: 'Syne', size: 10 }, color: 'rgba(22,15,8,0.4)' },
    },
    y: {
      beginAtZero: true,
      ticks: { stepSize: 1, font: { family: 'Syne', size: 10 }, color: 'rgba(22,15,8,0.4)' },
      grid: { color: 'rgba(22,15,8,0.04)' },
    },
  },
};
const hBarOpts = {
  ...
  scales: {
    ...
    y: {
      grid: { display: false },
      ticks: { font: { family: 'Syne', size: 11 }, color: 'rgba(22,15,8,0.5)' },
    },
  },
};
const donutOpts = {
...
function AutoInsight({ type = 'info', children }) {
  const map = {
    ...
    positive: {
      bg: 'rgba(30,122,74,0.08)',
      border: 'rgba(30,122,74,0.15)',
      color: 'var(--sage)',
      icon: (
        <svg
          width="10"
...
function OverviewTab({ analytics, trendDays, setTrendDays }) {
  ...
  const trendData = {
    ...
    datasets: [
      {
        label: 'Completed',
        data: responseTrend.map((d) => d.completed),
        borderColor: '#FF4500',
      ...
      {
        label: 'Started',
        data: responseTrend.map((d) => d.started),
        borderColor: '#0047FF',
        backgroundColor: 'rgba(0,71,255,0.04)',
        fill: true,
        tension: 0.45,
  ...
  const insights = [];
  if (total > 5 && completionRate < 50)
    insights.push({
      type: 'warning',
      msg: `Completion rate is only ${completionRate}% — survey may be too long or have friction points.`,
    });
  if (total >= 5 && completionRate >= 75)
    insights.push({
      type: 'positive',
      msg: `${completionRate}% completion rate is excellent — respondents find this survey engaging.`,
    });
  if (total > 5 && abandonRate > 30)
    insights.push({
      type: 'alert',
      msg: `${abandonRate}% abandon rate detected — check the Drop-off tab for the problematic question.`,
    });
  if (nps && nps.score < 0)
    insights.push({
      type: 'alert',
      msg: `NPS score of ${nps.score} is negative — detractors outnumber promoters. Urgent attention needed.`,
    });
  if (nps && nps.score >= 50)
    insights.push({
      type: 'positive',
      msg: `NPS of ${nps.score} (${nps.label}) — you're in the top tier for respondent satisfaction.`,
    });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      {insights.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {insights.map((ins, i) => (
            <AutoInsight key={i} type={ins.type}>
              {ins.msg}
            </AutoInsight>
          ))}
        </div>
      ...
      {hasTrend ? (
        ...
        >
          ...
          <div className="np-chart-wrap" style={{ height: 210 }}>
            <Line options={lineOpts} data={trendData} />
          </div>
        </motion.div>
      ) : (
        <div style={{ ...S.card, position: 'relative' }}>
          ...
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            <p style={{ ...S.body, color: 'rgba(22,15,8,0.40)', margin: 0 }}>
              No responses in the last {trendDays} days.
            </p>
          </div>
        </div>
      )}
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 20 }}>
                ...
                <span
                  style={{
                    ...
                    letterSpacing: '0.1em',
                    textTransform: 'uppercase',
                    color: npsColors[nps.label] || 'var(--coral)',
                  }}
                >
                  {nps.label}
                </span>
                ...
                <div style={{ width: 110, height: 110, flexShrink: 0 }}>
                  <Doughnut
                    ...
                    data={{
                      ...
                      datasets: [
                        {
                          data: devEntries.map(([, v]) => v),
                          backgroundColor: COLS.slice(0, devEntries.length),
                          borderWidth: 0,
                        },
                      ],
                    }}
                  ...
                  {devEntries.map(([key, val], i) => (
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div
                          style={{ width: 8, height: 8, borderRadius: '50%', background: COLS[i] }}
                        />
                        <span
                          style={{
                            fontFamily: 'Fraunces,serif',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {biggestDrop && biggestDrop.dropPct > 10 && (
          <AutoInsight type="alert">
            Largest drop-off at Q
            {dropOffFunnel.findIndex((s) => s.questionId === biggestDrop.questionId) + 1}: &ldquo;
            {biggestDrop.questionText.slice(0, 60)}&rdquo; — {biggestDrop.dropPct}% abandon here.
          </AutoInsight>
        )}
        {dropOffFunnel.filter((s) => s.dropPct === 0).length > dropOffFunnel.length * 0.7 && (
          <AutoInsight type="positive">
            Flow is strong — most questions have under 5% drop-off. Survey pacing is
            well-calibrated.
          </AutoInsight>
        )}
      </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {dropOffFunnel.map((step, i) => {
            ...
            const isWorst =
              biggestDrop && step.questionId === biggestDrop.questionId && step.dropPct > 10;
            return (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 3 }}>
                  ...
                  >
                    <motion.div
                      ...
                      style={{
                        height: '100%',
                        background: isWorst ? 'rgba(214,59,31,0.18)' : 'rgba(22,15,8,0.1)',
                        borderRadius: 8,
                        display: 'flex',
                        alignItems: 'center',
                        paddingLeft: 12,
                        border: isWorst ? '1px solid rgba(214,59,31,0.25)' : 'none',
                      }}
                    >
                      <span
                        style={{
                          ...
                          fontSize: 11,
                          fontWeight: 700,
                          color: isWorst ? 'var(--terracotta)' : 'rgba(22,15,8,0.5)',
                          whiteSpace: 'nowrap',
                          overflow: 'hidden',
                      ...
                      >
                        {step.questionText.slice(0, 60)}
                        {step.questionText.length > 60 ? '...' : ''}
                      </span>
                    </motion.div>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        fontSize: 11,
                        fontWeight: 700,
                        color: step.dropPct > 20 ? 'var(--terracotta)' : 'rgba(22,15,8,0.50)',
      ...
      {timingData.length > 0 && (
        <div style={S.card}>
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 14, flexWrap: 'wrap' }}>
            {[
              { l: 'Fast (<5s)', c: '#1E7A4A' },
              { l: 'Normal (5-15s)', c: '#FFB800' },
              { l: 'Slow (15-30s)', c: '#FF4500' },
              { l: 'Very slow (>30s)', c: '#D63B1F' },
            ].map((x) => (
            ...
            <tbody>
              {dropOffFunnel.map((s, i) => (
                <tr
                  key={s.questionId}
                  style={{
                    borderBottom: '1px solid rgba(22,15,8,0.04)',
                ...
                >
                  ...
                  >
                    {s.questionText.slice(0, 60)}
                    {s.questionText.length > 60 ? '...' : ''}
                  </td>
                  <td
                    style={{
                      padding: '9px 10px',
                  ...
                  <td
                    style={{
                      ...
                      fontWeight: 300,
                      fontSize: 13,
                      color: s.dropped > 0 ? 'var(--terracotta)' : 'rgba(22,15,8,0.3)',
                    }}
                  >
                    {s.dropped}
                  </td>
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={S.qResp}>
          {d?.total || 0} response{d?.total !== 1 ? 's' : ''}
        </div>
      </div>
      ...
      {!d ? (
            ...
            <Doughnut
              ...
              data={{
                ...
                datasets: [
                  {
                    data: d.values,
                    backgroundColor: COLS.slice(0, d.values.length),
                    borderWidth: 0,
                  },
                ],
              }}
              ...
              return (
                <div key={j}>
                  ...
                  >
                    ...
                    >
                      {d.values[j]} <span style={{ color: 'rgba(22,15,8,0.3)' }}>({pct}%)</span>
                    </span>
                  </div>
            ...
            <Bar
              ...
              data={{
                ...
                datasets: [
                  {
                    data: d.values,
                    backgroundColor: d.labels.map((_, j) => COLS[j % COLS.length]),
                    borderRadius: 6,
                    barThickness: 26,
                  },
                ],
        ...
        <div>
          ...
          >
            Avg rank — lower = ranked first by respondents
          </div>
            ...
            <tbody>
              {d.rows.map((row) => {
                const rowMax = Math.max(
                  ...d.cols.map((c) => d.matrix[row.value]?.[c.value] || 0),
                  1
                );
                return (
                  <tr key={row.value}>
                      ...
                      return (
                        <td
                          ...
                          style={{
                            padding: '6px 8px',
                            textAlign: 'center',
                            background: `rgba(255,69,0,${(intensity * 0.55).toFixed(2)})`,
                            borderRadius: 6,
                          }}
                        >
                          <span
                            style={{
                              ...
                              fontSize: 11,
                              fontWeight: 700,
                              color: intensity > 0.45 ? 'var(--cream)' : 'var(--espresso)',
                            }}
                          >
                            {val}
                          </span>
        ...
        <div>
          ...
          >
            {(d.items ?? []).slice(0, 18).map((r, j) => (
              <div key={j} style={S.textResp}>
                {r}
              </div>
            ))}
            {(d.items ?? []).length > 18 && (
        ...
        if (!d?.items?.length)
          return (
            <div key={q.id} style={S.card}>
              <div style={S.qNum}>
                {q.question_type === 'long_text' ? 'Long Text' : 'Short Text'}
              </div>
              <div style={S.qText}>{q.question_text}</div>
              <p style={{ ...S.body, color: 'rgba(22,15,8,0.50)', marginTop: 8 }}>
                No responses yet.
              </p>
            </div>
          );
        ...
        return (
          ...
          >
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              >
                {[
                  { v: pos, c: '#1E7A4A', l: 'Positive' },
                  { v: neu, c: 'rgba(22,15,8,0.12)', l: 'Neutral' },
                  { v: neg, c: '#D63B1F', l: 'Negative' },
                ]
                  .filter((s) => s.v > 0)
                  .map((s) => {
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                {[
                  { l: 'Positive', v: pos, c: 'var(--sage)' },
                  { l: 'Neutral', v: neu, c: 'rgba(22,15,8,0.4)' },
                  { l: 'Negative', v: neg, c: 'var(--terracotta)' },
                ].map((s) => (
                  <div key={s.l} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    >
                      {s.l}{' '}
                      <strong style={{ color: 'var(--espresso)', fontWeight: 600 }}>{s.v}</strong>
                    </span>
                  </div>
                ))}
              </div>
            ...
            {neg > pos && neg > d.total * 0.3 && (
              <div style={{ marginBottom: 20 }}>
                <AutoInsight type="warning">
                  {Math.round((neg / d.total) * 100)}% of responses carry negative sentiment. Review
                  keywords below for common complaints.
                </AutoInsight>
              </div>
            )}
            {pos > d.total * 0.6 && (
              <div style={{ marginBottom: 20 }}>
                <AutoInsight type="positive">
                  {Math.round((pos / d.total) * 100)}% of responses are positive — respondents are
                  satisfied here.
                </AutoInsight>
              </div>
            )}
      ...
      {!rs.length ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          ...
          {rs.length > 50 && (
            ...
            >
              Showing 50 of {rs.length} — export CSV for all
            </div>
          )}
        </div>
      )}
...
function FeedbackTab({ feedback }) {
  if (!feedback.length)
    return (
      <EmptyState message="No respondent feedback yet. It shows up after surveys are submitted." />
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 2, marginTop: 4 }}>
            {[1, 2, 3, 4, 5].map((i) => (
              <svg
                ...
                height="18"
                viewBox="0 0 24 24"
                fill={i <= Math.round(avg) ? '#FFB800' : 'none'}
                stroke="#FFB800"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <div style={{ ...S.statLbl, marginTop: 8 }}>
          ...
          {dist
            ...
            .map((d) => (
              <div key={d.star} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <svg width="11" height="11" viewBox="0 0 24 24" fill="#FFB800" stroke="none">
                  <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                </svg>
                <div
                  style={{ flex: 1, height: 7, background: 'var(--cream-deep)', borderRadius: 999 }}
                >
                  <motion.div
                    initial={{ width: 0 }}
...
export default function SurveyAnalytics() {
  ...
  async function load() {
    try {
      ...
      const allAnswers = [];
      r.forEach((resp) => {
        if (resp.survey_answers) allAnswers.push(...resp.survey_answers);
      });
      sAns(allAnswers);
  ...
  function exportPDF() {
    const { total, completedCount, abandonedCount, completionRate, avgTimeMin, milestones } =
      analytics;
    const qRows = qs
      .map((q, i) => {
    ...
    <div style={{ maxWidth: 940, margin: '0 auto' }}>
      ...
      <div className="np-page-header" style={{ marginBottom: 32 }}>
        <div>
          <Link
            to={`/surveys/${id}/edit`}
            style={{ ...S.backLink, display: 'flex', alignItems: 'center', gap: 5 }}
          ...
          >
            {sv.status.charAt(0).toUpperCase() + sv.status.slice(1)} · Created{' '}
            {formatDateTime(sv.created_at)}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button
            onClick={() => {
              ...
              import('react-hot-toast').then((m) => m.default.success('Analytics link copied!'));
            }}
            style={{ ...S.exportBtn, display: 'flex', alignItems: 'center', gap: 5 }}
            onMouseEnter={(e) => {
            ...
            onMouseLeave={(e) => {
              ...
              e.currentTarget.style.color = 'rgba(22,15,8,0.55)';
            }}
          >
            <svg
              width="11"
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {tab === 'Overview' && (
            <OverviewTab analytics={analytics} trendDays={trendDays} setTrendDays={setTrendDays} />
          )}
