...
function parseOpts(raw, forMatrix = false) {
  ...
  if (typeof raw === 'string') {
    try {
      ...
      return forMatrix ? { rows: [], columns: [] } : [];
    }
  }
  if (forMatrix)
    return raw && !Array.isArray(raw) && typeof raw === 'object' ? raw : { rows: [], columns: [] };
  return Array.isArray(raw) ? raw : [];
}

const fi = (e) => {
  e.target.style.borderColor = 'var(--coral)';
  e.target.style.boxShadow = '0 0 0 3px rgba(255,69,0,0.08)';
};
const fo = (e) => {
...
const STATUS_COLORS = {
  draft: { bg: 'rgba(22,15,8,0.07)', text: 'rgba(22,15,8,0.45)', dot: 'rgba(22,15,8,0.3)' },
  active: { bg: 'rgba(30,122,74,0.1)', text: 'var(--sage)', dot: 'var(--sage)' },
  paused: { bg: 'rgba(255,184,0,0.12)', text: '#A07000', dot: 'var(--saffron)' },
  closed: { bg: 'rgba(214,59,31,0.08)', text: 'var(--terracotta)', dot: 'var(--terracotta)' },
};
...
export default function SurveyEdit() {
  ...
  async function load() {
    try {
      ...
      sQs(
        (q || []).map((x) => {
          const opts = isMx(x.question_type)
            ? parseOpts(x.options, true)
            : hasO(x.question_type)
              ? parseOpts(x.options)
              : x.options;
  ...
  const addQ = () =>
    sQs((a) => [
      ...a,
      {
        _id: 'new_' + Math.random().toString(36).slice(2),
        question_text: '',
  ...
  const moveQ = (tid, d) =>
    sQs((a) => {
      const i = a.findIndex((q) => q._id === tid);
      if ((d === -1 && i === 0) || (d === 1 && i === a.length - 1)) return a;
      const b = [...a];
      [b[i], b[i + d]] = [b[i + d], b[i]];
      return b;
    });
  const addOpt = (tid) =>
    sQs((a) =>
      a.map((q) =>
        q._id === tid ? { ...q, options: [...(q.options || []), { label: '', value: '' }] } : q
      )
    );
  const sOpt = (tid, i, v) =>
    sQs((a) =>
      a.map((q) => {
        if (q._id !== tid) return q;
        const o = [...(q.options || [])];
        o[i] = { label: v, value: v.toLowerCase().replace(/\s+/g, '_') };
        return { ...q, options: o };
      })
    );
  const delOpt = (tid, i) =>
    sQs((a) =>
      a.map((q) => (q._id !== tid ? q : { ...q, options: q.options.filter((_, j) => j !== i) }))
    );
  ...
  async function save() {
    ...
    try {
      const meta = {
        ...
        welcome_message: sv.welcome_message || null,
        thank_you_message: sv.thank_you_message || null,
  ...
  function copyLink() {
    ...
    navigator.clipboard.writeText(`${appOrigin}/s/${sv.slug}`);
  ...
  if (!sv)
    return (
      <div style={{ textAlign: 'center', padding: '100px 0' }}>
        ...
        <div style={{ fontFamily: "'Fraunces',serif", color: 'rgba(22,15,8,0.35)', fontSize: 15 }}>
          Survey not found
        </div>
      </div>
    );
  ...
  const healthColor =
    health >= 80 ? 'var(--sage)' : health >= 50 ? 'var(--saffron)' : 'var(--terracotta)';
  ...
  const TABS = [
    ...
    { id: 'settings', n: '03', label: 'Settings' },
  ];
  ...
  const ARC_R = 28,
    ...
    arcOffset = ARC_CIRC - (health / 100) * ARC_CIRC;
        ...
        >
          <div
            ...
            style={{
              ...
              bottom: 0,
              width: 3,
              background: `linear-gradient(180deg,${tc},${tc}40)`,
              opacity: 0.4,
          ...
          <div style={{ padding: '24px 28px 22px 32px' }}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.color = 'rgba(22,15,8,0.5)';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = 'none';
                    e.currentTarget.style.color = 'rgba(22,15,8,0.2)';
                  }}
                >
              ...
              <div style={{ display: 'flex', gap: 2 }}>
                {[
                  ...
                  <button
                    key={d}
                    onClick={() => moveQ(q._id, d)}
                    disabled={(d === -1 && i === 0) || (d === 1 && i === qs.length - 1)}
                    ...
                    style={{
                      ...
                      justifyContent: 'center',
                      transition: 'all 0.15s',
                      opacity: (d === -1 && i === 0) || (d === 1 && i === qs.length - 1) ? 0.18 : 1,
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.background = 'var(--cream-deep)';
                    ...
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = 'none';
                      e.currentTarget.style.color = 'rgba(22,15,8,0.25)';
                    }}
                  >
                    {sym}
                  </button>
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = 'rgba(214,59,31,0.08)';
            ...
            <input
              ...
              onChange={(e) => sQ(q._id, 'question_text', e.target.value)}
              placeholder="Type your question here…"
              style={{
                ...INP,
                fontSize: 17,
                padding: '14px 18px',
                background: 'rgba(253,245,232,0.55)',
                border: '1.5px solid rgba(22,15,8,0.07)',
                marginBottom: 10,
                borderRadius: 16,
              }}
            ...
            <input
              value={q.description || ''}
              onChange={(e) => sQ(q._id, 'description', e.target.value)}
              ...
              style={{
                ...
                border: '1.5px solid rgba(22,15,8,0.06)',
                marginBottom: 16,
                borderRadius: 13,
              }}
              onFocus={fi}
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                ...
                {typeOpen && (
                  <div
                    style={{
                      ...
                      borderRadius: 16,
                      padding: 6,
                      boxShadow: '0 24px 60px rgba(22,15,8,0.3)',
                      maxHeight: 280,
                      overflowY: 'auto',
                    }}
                  >
                    {QUESTION_TYPES.map((t) => (
                      <button
                        key={t.value}
                        onClick={() => {
                          sQ(q._id, 'question_type', t.value);
                          setTypeOpen(false);
                        }}
                        style={{
                          width: '100%',
                          ...
                          color:
                            t.value === q.question_type ? 'var(--coral)' : 'rgba(253,245,232,0.75)',
                          textAlign: 'left',
                        }}
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.background = 'rgba(253,245,232,0.08)')
                        }
              ...
              >
                <div
                  onClick={() => sQ(q._id, 'is_required', !q.is_required)}
                  style={{
                    width: 38,
                    height: 22,
                    borderRadius: 999,
                    background: q.is_required ? tc : 'rgba(22,15,8,0.12)',
                    position: 'relative',
                ...
                >
                  <div
                    style={{
                      ...
                      background: '#fff',
                      top: 3,
                      left: q.is_required ? 19 : 3,
                      transition: 'left 0.25s',
                      boxShadow: '0 1px 4px rgba(22,15,8,0.2)',
                    }}
                  />
                </div>
                <span
                  ...
                  {(q.options || []).map((o, j) => (
                    <div
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        border: '1.5px solid rgba(22,15,8,0.07)',
                        background: 'rgba(253,245,232,0.5)',
                        transition: 'all 0.15s',
                      }}
                    >
                      <div
                      ...
                      <input
                        value={o.label}
                        onChange={(e) => sOpt(q._id, j, e.target.value)}
                        placeholder={`Option ${j + 1}`}
                        className="opt-input"
                      />
                      <button
                        onClick={() => delOpt(q._id, j)}
                        style={{
                          background: 'none',
                          border: 'none',
                          cursor: 'pointer',
                          color: 'rgba(22,15,8,0.18)',
                          fontSize: 12,
                          padding: 4,
                        ...
                        onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(22,15,8,0.18)')}
                      >
                        ✕
                      </button>
                    </div>
            ...
            {isMx(q.question_type) &&
              (() => {
                const mx =
                  q.options && !Array.isArray(q.options) ? q.options : { rows: [], columns: [] };
                ...
                const addRow = () =>
                  setMx({
                    ...
                    rows: [
                      ...(mx.rows || []),
                      {
                        label: `Row ${(mx.rows || []).length + 1}`,
                        value: `row_${(mx.rows || []).length + 1}`,
                      },
                    ],
                  });
                const addCol = () =>
                  setMx({
                    ...
                    columns: [
                      ...(mx.columns || []),
                      {
                        label: `Col ${(mx.columns || []).length + 1}`,
                        value: `col_${(mx.columns || []).length + 1}`,
                      },
                    ],
                  });
                const updRow = (ri, v) => {
                  const r = [...(mx.rows || [])];
                  r[ri] = { label: v, value: v.toLowerCase().replace(/\s+/g, '_') };
                  setMx({ ...mx, rows: r });
                };
                const updCol = (ci, v) => {
                  const cs = [...(mx.columns || [])];
                  cs[ci] = { label: v, value: v.toLowerCase().replace(/\s+/g, '_') };
                  setMx({ ...mx, columns: cs });
                };
                const delRow = (ri) =>
                  setMx({ ...mx, rows: (mx.rows || []).filter((_, j) => j !== ri) });
                const delCol = (ci) =>
                  setMx({ ...mx, columns: (mx.columns || []).filter((_, j) => j !== ci) });
                return (
                  <div
                    className="mx-grid"
                    style={{
                  ...
                  >
                    {[
                      ['Rows', mx.rows || [], addRow, updRow, delRow],
                      ['Columns', mx.columns || [], addCol, updCol, delCol],
                    ].map(([lbl, items, add, upd, del]) => (
                      <div key={lbl}>
                        <div
                          style={{
                            fontFamily: "'Syne',sans-serif",
                        ...
                        {items.map((r, idx) => (
                          <div key={idx} style={{ display: 'flex', gap: 7, marginBottom: 7 }}>
                            <input
                              value={r.label}
                              onChange={(e) => upd(idx, e.target.value)}
                              placeholder={`${lbl.slice(0, -1)} ${idx + 1}`}
                              style={{
                                ...INP,
                                flex: 1,
                                padding: '9px 13px',
  ...
  return (
    <div>
        ...
        >
          <div
            style={{
              ...
              maxHeight: '88vh',
              overflow: 'auto',
              boxShadow: '0 64px 160px rgba(22,15,8,0.5)',
              position: 'relative',
            }}
          >
            <div
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = 'rgba(22,15,8,0.06)';
                    e.currentTarget.style.color = 'var(--espresso)';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = 'none';
                    e.currentTarget.style.color = 'rgba(22,15,8,0.35)';
                  }}
                >
                  ✕
                </button>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (sec === 'Questions')
                          setPreviewStep(Math.max(0, Math.min(previewStep, qs.length - 1)));
                      ...
                      style={{
                        ...
                        color: active ? 'var(--cream)' : 'rgba(22,15,8,0.35)',
                        boxShadow: active ? '0 2px 10px rgba(22,15,8,0.15)' : 'none',
                      }}
                    >
                      {sec}
                    </button>
            ...
            <div style={{ padding: 36 }}>
                ...
                (() => {
                  ...
                  return (
                    <div>
                      ...
                      >
                        {q.question_text || <em style={{ opacity: 0.3 }}>No question text</em>}
                      </div>
                      {q.description && (
                        <div
                          style={{
                        ...
                        >
                          {parseOpts(q.options).map((o, j) => (
                            <div
                              ...
                              onMouseLeave={(e) => {
                                e.currentTarget.style.borderColor = 'rgba(22,15,8,0.09)';
                            ...
                            >
                              <div
                                style={{
                                  width: 16,
                                  height: 16,
                                  borderRadius: q.question_type === 'multiple_choice' ? 5 : '50%',
                                  border: `2px solid rgba(22,15,8,0.2)`,
                                  flexShrink: 0,
                                }}
                              />
                              <span
                      ...
                      <div
                        style={{
                          ...
                          marginTop: 28,
                          paddingTop: 20,
                          borderTop: '1px solid rgba(22,15,8,0.06)',
                        }}
                      >
                        <button
                          ...
                          style={{
                            ...
                            textTransform: 'uppercase',
                            color: 'rgba(22,15,8,0.45)',
                            cursor: qi === 0 ? 'not-allowed' : 'pointer',
                            opacity: qi === 0 ? 0.3 : 1,
                            transition: 'all 0.2s',
                          }}
                        >
                        ...
                        <button
                          onClick={() => setPreviewStep((p) => Math.min(qs.length, p + 1))}
                          style={{
                            padding: '11px 28px',
                            borderRadius: 999,
                            border: 'none',
              ...
              {curSection === 'Post Survey' && (
                <div style={{ textAlign: 'center', padding: '24px 0' }}>
                  ...
                  >
                    {sv.thank_you_message || 'Thank you for completing this survey!'}
                  </p>
                </div>
              )}
            </div>
      ...
      <ConfirmModal
        ...
        body="This survey has expired. Choose how many days to extend it."
      ...
      <ConfirmModal
        open={deleteOpen}
        title="Delete Survey"
        body="This action cannot be undone. All responses will be permanently deleted."
        confirmLabel="Delete"
        danger
        onConfirm={() => {
          doDelete();
      ...
      />
      <ShareModal
        survey={{ slug: sv.slug, title: sv.title }}
        isOpen={pubShareOpen}
      ...
      {/* ── PAGE HEADER ── */}
      <div
        style={{ position: 'relative', marginBottom: 48, paddingBottom: 44, overflow: 'hidden' }}
      >
        <div
          style={{
            position: 'absolute',
        ...
        <div
          style={{
            ...
            height: 360,
            borderRadius: '50%',
            background: `radial-gradient(circle,${tc}20,transparent 70%)`,
            pointerEvents: 'none',
          }}
        />
        <div
          style={{
            ...
            height: '1px',
            background:
              'linear-gradient(90deg,transparent,rgba(22,15,8,0.08) 30%,rgba(22,15,8,0.08) 70%,transparent)',
          }}
        />
          ...
          <div className="np-page-header" style={{ marginBottom: 32 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <div
                  style={{ width: 28, height: 1.5, background: 'var(--coral)', borderRadius: 1 }}
                />
                <span
                  style={{
                    fontFamily: "'Syne',sans-serif",
            ...
            <div style={{ display: 'flex', gap: 8, flexShrink: 0, flexWrap: 'wrap' }}>
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.borderColor = 'rgba(22,15,8,0.25)';
                  e.currentTarget.style.color = 'var(--espresso)';
                  e.currentTarget.style.background = 'rgba(255,255,255,0.9)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.borderColor = 'rgba(22,15,8,0.12)';
                  e.currentTarget.style.color = 'rgba(22,15,8,0.5)';
              ...
              {isEditing && (
                <button
                  ...
                  onMouseEnter={(e) => {
                    if (!busy) {
                      e.currentTarget.style.background = tc;
                      e.currentTarget.style.boxShadow = `0 10px 36px ${tc}50`;
                    }
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = 'var(--espresso)';
                    e.currentTarget.style.boxShadow = '0 6px 24px rgba(22,15,8,0.25)';
                  }}
                >
                  {busy ? (
                    'Saving…'
              ...
              return (
                <button
                  ...
                  style={{
                    padding: '5px 14px',
                    borderRadius: 999,
                    border: `1.5px solid ${sv.status === st ? sc.text : 'rgba(22,15,8,0.1)'}`,
                    background: sv.status === st ? sc.bg : 'transparent',
                    fontFamily: "'Syne',sans-serif",
                    fontWeight: 700,
                    fontSize: 9,
                    letterSpacing: '0.12em',
                    textTransform: 'uppercase',
                    color: sv.status === st ? sc.text : 'rgba(22,15,8,0.35)',
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                  onMouseEnter={(e) => {
                    if (sv.status !== st) {
                      e.currentTarget.style.borderColor = sc.text;
                      e.currentTarget.style.color = sc.text;
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (sv.status !== st) {
                      e.currentTarget.style.borderColor = 'rgba(22,15,8,0.1)';
                      e.currentTarget.style.color = 'rgba(22,15,8,0.35)';
                    }
                  }}
                >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.background = `${tc}06`;
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.borderColor = 'rgba(22,15,8,0.1)';
              e.currentTarget.style.color = 'rgba(22,15,8,0.45)';
          ...
          {sv.status === 'draft' && hasPermission(profile?.role, 'delete_survey') && (
            <button
              ...
              style={{
                padding: '7px 16px',
                borderRadius: 999,
                border: '1.5px solid rgba(214,59,31,0.15)',
                background: 'transparent',
                fontFamily: "'Syne',sans-serif",
                fontWeight: 700,
                fontSize: 9,
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.color = 'var(--terracotta)';
                e.currentTarget.style.background = 'rgba(214,59,31,0.05)';
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = 'rgba(214,59,31,0.15)';
                e.currentTarget.style.color = 'rgba(214,59,31,0.5)';
                e.currentTarget.style.background = 'transparent';
              }}
            >
              Delete
      ...
      <div
        className="se-grid np-grid-responsive"
            ...
            {TABS.map((t) => (
              <button
                ...
                style={{
                  ...
                  letterSpacing: '0.14em',
                  textTransform: 'uppercase',
                  color: tab === t.id ? 'var(--espresso)' : 'rgba(22,15,8,0.32)',
                  transition: 'color 0.2s',
                  marginRight: 4,
                }}
              >
                <span
                  style={{
                    ...
                    color: tab === t.id ? tc : 'rgba(22,15,8,0.2)',
                    transition: 'color 0.2s',
                  }}
                >
                  {t.n}
                ...
                {t.count !== undefined && (
                  <span
                    style={{
                      ...
                      height: 18,
                      borderRadius: 999,
                      background: tab === t.id ? `${tc}15` : 'rgba(22,15,8,0.07)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      padding: '0 5px',
                      ...
                      fontFamily: "'Syne',sans-serif",
                      fontWeight: 700,
                      color: tab === t.id ? tc : 'rgba(22,15,8,0.35)',
                    }}
                  >
                    {t.count}
                  </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
              ...
              >
                  ...
                  {isEditing ? (
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 300,
                        fontSize: 14,
                        color: sv.description ? 'var(--espresso)' : 'rgba(22,15,8,0.3)',
                        padding: '12px 18px',
                        background: 'var(--cream-deep)',
                        borderRadius: 16,
                        minHeight: 48,
                ...
                <div>
                  ...
                  {isEditing ? (
                    <textarea
                      value={sv.welcome_message || ''}
                      onChange={(e) => s('welcome_message', e.target.value)}
                      placeholder="Shown before Q1"
                      rows={4}
                      style={{ ...INP, borderRadius: 16 }}
                      onFocus={fi}
                      onBlur={fo}
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 300,
                        fontSize: 14,
                        color: sv.welcome_message ? 'var(--espresso)' : 'rgba(22,15,8,0.3)',
                        padding: '12px 18px',
                        background: 'var(--cream-deep)',
                        borderRadius: 16,
                        minHeight: 48,
              ...
              <div>
                ...
                {isEditing ? (
                  <textarea
                    value={sv.thank_you_message || ''}
                    onChange={(e) => s('thank_you_message', e.target.value)}
                ...
                <div>
                  ...
                  {isEditing ? (
                    <input
                      type="datetime-local"
                      value={sv.expires_at || ''}
                      onChange={(e) => s('expires_at', e.target.value)}
                      style={{ ...INP, borderRadius: 16 }}
                      onFocus={fi}
                      onBlur={fo}
                    />
                    ...
                    <div
                      style={{
                        ...
                        fontWeight: 300,
                        fontSize: 14,
                        color: sv.expires_at ? 'var(--espresso)' : 'rgba(22,15,8,0.3)',
                        padding: '12px 18px',
                        background: 'var(--cream-deep)',
                        borderRadius: 16,
                        minHeight: 48,
                    ...
                    >
                      {sv.expires_at ? formatDate(sv.expires_at) : 'No expiry set'}
                    </div>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <input
                        type="color"
                        value={sv.theme_color || '#FF4500'}
                        onChange={(e) => s('theme_color', e.target.value)}
                        style={{
                          width: 52,
                          height: 52,
                          borderRadius: 14,
                      ...
                      />
                      <input
                        value={sv.theme_color || ''}
                        onChange={(e) => s('theme_color', e.target.value)}
                        style={{ ...INP, flex: 1, letterSpacing: '0.05em', borderRadius: 16 }}
                        onFocus={fi}
                        onBlur={fo}
                      />
                    </div>
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 28,
                          borderRadius: 9,
                          background: sv.theme_color || '#FF4500',
                          flexShrink: 0,
                          boxShadow: `0 2px 8px ${tc}40`,
                        }}
                      />
                      <span
                        style={{
              ...
              {!isEditing && (
                <button
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow = `0 6px 24px ${tc}40`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = 'transparent';
                    e.currentTarget.style.color = tc;
                    ...
                    >
                      <div
                        style={{
                          ...
                          bottom: 0,
                          width: 3,
                          background: `linear-gradient(180deg,${tc},${tc}55)`,
                          opacity: 0.35,
                        }}
                      />
                      <div
                      ...
                      <div style={{ paddingLeft: 4, position: 'relative', zIndex: 1 }}>
                        ...
                        <p
                          style={{
                            ...
                            fontSize: 18,
                            color: 'var(--espresso)',
                            marginBottom: q.description ? 8 : 0,
                            lineHeight: 1.35,
                            letterSpacing: '-0.2px',
                          }}
                        >
                ...
                <>
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.borderColor = tc;
                      e.currentTarget.style.color = tc;
                      e.currentTarget.style.background = `${tc}05`;
                      e.currentTarget.style.boxShadow = `0 4px 24px ${tc}10`;
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.borderColor = 'rgba(22,15,8,0.1)';
                      e.currentTarget.style.color = 'rgba(22,15,8,0.28)';
                      e.currentTarget.style.background = 'transparent';
                      e.currentTarget.style.boxShadow = 'none';
                    }}
                  >
          ...
          {tab === 'settings' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                {
                  k: 'allow_anonymous',
                  l: 'Anonymous responses',
                  d: 'Respondents don&apos;t need to identify themselves',
                  ico: (
                    <svg
                      width="16"
                      height="16"
                ...
                {
                  k: 'require_email',
                  l: 'Require email address',
                  d: 'Collect respondent emails before they begin',
                  ico: (
                    <svg
                      width="16"
                      height="16"
                ...
                },
                {
                  k: 'show_progress_bar',
                  l: 'Show progress bar',
                  d: 'Display a completion indicator to respondents',
                  ico: (
                    <svg
                      width="16"
                      height="16"
                ...
                <div
                  ...
                  onClick={() => {
                    if (isEditing) s(x.k, !sv[x.k]);
                  }}
                  onMouseEnter={(e) => {
                    if (isEditing) {
                      e.currentTarget.style.borderColor = 'rgba(22,15,8,0.14)';
                      e.currentTarget.style.background = '#fff';
                      e.currentTarget.style.boxShadow = '0 6px 28px rgba(22,15,8,0.06)';
                    }
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.borderColor = 'rgba(22,15,8,0.07)';
                    e.currentTarget.style.background = 'var(--warm-white)';
                    e.currentTarget.style.boxShadow = 'none';
                  }}
                >
                  <div
                    style={{
                      ...
                      bottom: 0,
                      width: 3,
                      background: sv[x.k] ? `linear-gradient(180deg,${tc},${tc}50)` : 'transparent',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 18, paddingLeft: 8 }}>
                    <div
                      style={{
                        ...
                        height: 44,
                        borderRadius: 14,
                        background: sv[x.k] ? `${tc}12` : 'rgba(22,15,8,0.05)',
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        color: sv[x.k] ? tc : 'rgba(22,15,8,0.32)',
                        transition: 'all 0.25s',
                        flexShrink: 0,
                      }}
                    >
                  ...
                  <div
                    style={{
                      ...
                      height: 26,
                      borderRadius: 999,
                      background: sv[x.k] ? tc : 'rgba(22,15,8,0.12)',
                      position: 'relative',
                      transition: 'background 0.25s',
                      flexShrink: 0,
                  ...
                  >
                    <div
                      style={{
                        ...
                        left: sv[x.k] ? 23 : 3,
                        transition: 'left 0.25s',
                        boxShadow: '0 1px 6px rgba(22,15,8,0.2)',
                      }}
                    />
                  </div>
                </div>
              ...
              {!isEditing && (
                ...
                >
                  Read-only — click &quot;Edit Survey&quot; to make changes.
                </p>
              )}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                  ...
                  <div>
                    ...
                    >
                      Share this survey with your team
                    </div>
                  </div>
                </div>
                ...
                {isEditing && (
                  <div
                    style={{
                      ...
                      marginBottom: 24,
                      paddingBottom: 24,
                      borderBottom: '1px solid rgba(22,15,8,0.05)',
                    }}
                  >
                    <select
                      id="team-member-select"
                      onChange={(e) => {
                        if (e.target.value) share(e.target.value);
                        e.target.value = '';
                      }}
                      style={{
                        ...INP,
                    ...
                    >
                      ...
                      {users
                        .filter(
                          (u) => u.id !== profile.id && !shares.some((s) => s.shared_with === u.id)
                        )
                        .map((u) => (
                      ...
                      >
                        ...
                        {isEditing && (
                          <button
                            ...
                            onMouseLeave={(e) =>
                              (e.currentTarget.style.color = 'rgba(214,59,31,0.4)')
                            }
                          >
                            Revoke
                          </button>
        ...
        >
          ...
          <div
            style={{
              ...
              borderRadius: 24,
              overflow: 'hidden',
              boxShadow: '0 16px 56px rgba(22,15,8,0.25)',
              position: 'relative',
            }}
          >
            <div
              style={{
                ...
                height: 160,
                borderRadius: '50%',
                background: `radial-gradient(circle,${tc}30,transparent 70%)`,
                pointerEvents: 'none',
              }}
            />
            ...
            <div style={{ padding: '20px 22px 24px', position: 'relative', zIndex: 1 }}>
              ...
              <div
                style={{
                  ...
                  color: 'var(--cream)',
                  lineHeight: 1.15,
                  marginBottom: sv.description ? 8 : 0,
                }}
              >
                {sv.title}
              </div>
              ...
              <div
                style={{
                  ...
                  marginTop: 18,
                  paddingTop: 16,
                  borderTop: '1px solid rgba(255,251,244,0.08)',
                }}
              >
                {[
                  [`${qs.length}`, 'questions'],
                  [`${qs.filter((q) => q.is_required).length}`, 'required'],
                ].map(([v, l]) => (
                  <div
                    key={l}
                    style={{
                      flex: 1,
                      textAlign: 'center',
                      borderRight: l !== 'required' ? '1px solid rgba(255,251,244,0.08)' : 'none',
                    }}
                  >
                    <div
                      style={{
            ...
            >
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  ...
                  <HelpTip
                    text="Improve by adding a welcome message, setting an expiry, using varied question types, and keeping surveys under 15 questions."
                    position="bottom"
                  />
                </div>
                    ...
                    <div key={tip} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                      <div
                        style={{
                          ...
                          borderRadius: '50%',
                          flexShrink: 0,
                          background: done ? 'var(--sage)' : 'rgba(22,15,8,0.08)',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          transition: 'background 0.25s',
                      ...
                      <span
                        style={{
                          ...
                          fontWeight: 300,
                          fontSize: 12,
                          color: done ? 'rgba(22,15,8,0.32)' : 'rgba(22,15,8,0.5)',
            ...
            {isEditing && (
              <button
                ...
                style={{
                  ...
                  boxShadow: '0 6px 28px rgba(22,15,8,0.2)',
                  opacity: busy ? 0.5 : 1,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                ...
                onMouseEnter={(e) => {
                  if (!busy) {
                    e.currentTarget.style.background = tc;
                    e.currentTarget.style.boxShadow = `0 10px 40px ${tc}45`;
                  }
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = 'var(--espresso)';
                  e.currentTarget.style.boxShadow = '0 6px 28px rgba(22,15,8,0.2)';
                }}
              >
                {busy ? (
                  'Saving…'
