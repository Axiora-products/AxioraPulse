...
function AnimatedStat({ val, label, accent }) {
  ...
  return (
    <>
      <div style={S.statNum} className="count-up">
        {display}
      </div>
...
export default function Dashboard() {
  ...
  async function load() {
    try {
      ...
      const completions =
        s.total_responses > 0 ? Math.round((s.completion_rate / 100) * s.total_responses) : 0;
      setStats({
      ...
      {recent.length === 0 ? (
        <div style={S.emptyState}>
          ...
          <p style={S.emptyBody}>
            ...
            <br />
            Create your first survey and see insights flow in.
          </p>
          {hasPermission(profile?.role, 'create_survey') && (
            <Link
              to="/surveys/new"
                ...
                >
                  <div
                    style={{
                      ...
                      boxShadow: `0 0 8px ${sv.theme_color || 'rgba(255,69,0,0.4)'}`,
                    }}
                  />
                  <span
                    className={`p-badge ${SURVEY_STATUS[sv.status]?.class || 'p-badge-draft'}`}
