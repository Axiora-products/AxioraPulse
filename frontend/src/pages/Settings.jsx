...
function ApprovedDomainsCard({ tenant, onSaved }) {
  const [domains, setDomains] = React.useState(() => (tenant?.approved_domains || []).join(', '));
  ...
  return (
    <div style={card}>
      ...
      >
        Invitations can only be sent to addresses belonging to these domains. You must set at least
        one domain before any invites can be sent.
      </p>
      ...
      {approvedList.length === 0 && (
        ...
        >
          ...
          >
            No domains set — invitations are blocked until you add at least one.
          </span>
        </div>
      )}
      ...
      <form onSubmit={save} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          >
            Enter domains without the @ sign, e.g. <em>acme.com</em>. Separate multiple domains with
            commas.
          </p>
        </div>
        <div>
          <button
            ...
            onMouseEnter={(e) => {
              if (!saving) e.currentTarget.style.background = 'var(--coral)';
            }}
            onMouseLeave={(e) => {
              if (!saving) e.currentTarget.style.background = 'var(--espresso)';
            }}
          >
...
export default function Settings() {
  ...
  function syncDomains(domains) {
    useAuthStore.setState((s) => ({
      tenant: s.tenant ? { ...s.tenant, approved_domains: domains } : s.tenant,
    }));
  }
  ...
  async function saveP(e) {
    ...
    sSP(true);
    try {
      await updateProfile({ full_name: pF.full_name });
      ...
      <div style={card}>
        ...
        <form onSubmit={saveP} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            ...
            <input
              value={pF.full_name}
              onChange={(e) => sPF({ ...pF, full_name: e.target.value })}
              style={inp}
              onFocus={(e) => (e.target.style.borderColor = 'var(--coral)')}
              onBlur={(e) => (e.target.style.borderColor = 'rgba(22,15,8,0.1)')}
            />
          ...
          <div>
            <button
              ...
              style={btn}
              onMouseEnter={(e) => {
                if (!sP) e.currentTarget.style.background = 'var(--coral)';
              }}
              onMouseLeave={(e) => {
                if (!sP) e.currentTarget.style.background = 'var(--espresso)';
              }}
            >
        ...
        <form onSubmit={savePw} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div>
            ...
            <input
              type="password"
              value={pwF.next}
              onChange={(e) => setPwF((p) => ({ ...p, next: e.target.value }))}
          ...
          <div>
            ...
            <input
              type="password"
              value={pwF.confirm}
              onChange={(e) => setPwF((p) => ({ ...p, confirm: e.target.value }))}
          ...
          <div>
            <button
              type="submit"
              disabled={sPw || !pwF.next || !pwF.confirm}
              style={{ ...btn, opacity: sPw || !pwF.next || !pwF.confirm ? 0.5 : 1 }}
              onMouseEnter={(e) => {
                if (!sPw) e.currentTarget.style.background = 'var(--coral)';
              }}
              onMouseLeave={(e) => {
                if (!sPw) e.currentTarget.style.background = 'var(--espresso)';
              }}
            >
        ...
        <div style={card}>
          ...
          <form onSubmit={saveT} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              ...
              <input
                value={tF.name}
                onChange={(e) => sTF({ ...tF, name: e.target.value })}
                style={inp}
                onFocus={(e) => (e.target.style.borderColor = 'var(--coral)')}
                onBlur={(e) => (e.target.style.borderColor = 'rgba(22,15,8,0.1)')}
              />
            ...
            <div>
              ...
              <div style={{ ...dis, display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 10, opacity: 0.5 }}>
                  .Axiora.io
                </span>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <input
                  type="color"
                  value={tF.primary_color}
                  onChange={(e) => sTF({ ...tF, primary_color: e.target.value })}
                  style={{
                    width: 48,
                    height: 48,
                    borderRadius: 12,
            ...
            <div>
              <button
                ...
                style={btn}
                onMouseEnter={(e) => {
                  if (!sT) e.currentTarget.style.background = 'var(--coral)';
                }}
                onMouseLeave={(e) => {
                  if (!sT) e.currentTarget.style.background = 'var(--espresso)';
                }}
              >
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 999,
              border: 'none',
              background: dark ? 'var(--coral)' : 'rgba(22,15,8,0.15)',
              cursor: 'pointer',
              position: 'relative',
