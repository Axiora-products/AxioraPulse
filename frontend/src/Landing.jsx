...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const interactables = document.querySelectorAll(
      'a, button, .lp-step, .lp-tcard, .lp-pcard, .lp-qt-chip, .lp-q-card, .lp-kpi-box'
    );
    interactables.forEach((el) => {
      el.addEventListener('mouseenter', () =>
        document.documentElement.classList.add('lp-hovering')
      );
      el.addEventListener('mouseleave', () =>
        document.documentElement.classList.remove('lp-hovering')
      );
    });
    document.addEventListener('mousedown', () =>
      document.documentElement.classList.add('lp-clicking')
    );
    document.addEventListener('mouseup', () =>
      document.documentElement.classList.remove('lp-clicking')
    );
    ...
    <div className="lp">
      ...
      <nav className={`lp-nav${stuck ? ' stuck' : ''}`}>
        ...
        <ul className="lp-nav-links">
          ...
          <li>
            <button className="lp-nav-btn" onClick={onEnterApp}>
              Sign In
            </button>
          </li>
        </ul>
        ...
        <div className="lp-pricing-grid">
          {[
            ...
            <div className={`lp-pcard ${p.cls} lp-sr lp-sr-d${i + 1}`} key={p.cls}>
              ...
              <button className="lp-pcard-btn" onClick={onEnterApp}>
                {p.btn}
              </button>
            </div>
          ))}
