...
export default function ShareModal({ survey, isOpen, onClose }) {
  ...
  async function sendEmail() {
    ...
    try {
      ...
      const msg = error.response?.data?.detail || 'Failed to send survey';
          ...
          >
              ...
              {TABS.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    textTransform: 'uppercase',
                    transition: 'all 0.2s',
                    background: tab === t.id ? 'var(--espresso)' : 'transparent',
                    color: tab === t.id ? 'var(--cream)' : 'rgba(22,15,8,0.4)',
                  }}
                >
                  {t.label}
                </button>
            ...
            {tab === 'link' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <motion.button
                    ...
                    style={{
                      ...btnPrimary(false),
                      minWidth: 80,
                      background: copied ? 'var(--sage)' : 'var(--espresso)',
                    }}
                    onMouseEnter={(e) => {
                      if (!copied) e.currentTarget.style.background = 'var(--coral)';
                    }}
                    onMouseLeave={(e) => {
                      if (!copied) e.currentTarget.style.background = 'var(--espresso)';
                    }}
                  >
                    {copied ? '✓ Copied' : 'Copy'}
                  </motion.button>
                ...
                >
                  Share this link directly. Respondents don&apos;t need an account to take the
                  survey.
                </p>
              </div>
            )}
            ...
            {tab === 'qr' && (
              ...
              >
                ...
                <button
                  onClick={() => {
                    const img =
                      document.querySelector('#nx-qr-img') ||
                      document.querySelector('img[alt="QR code"]');
                    if (!img) return;
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {EMBED_SIZES.map((s, i) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: '8px 0',
                        borderRadius: 10,
                        border: `1.5px solid ${embedSize === i ? 'var(--espresso)' : 'rgba(22,15,8,0.1)'}`,
                        background: embedSize === i ? 'var(--espresso)' : 'transparent',
                        color: embedSize === i ? 'var(--cream)' : 'rgba(22,15,8,0.45)',
                        fontFamily: 'Syne,sans-serif',
                        fontWeight: 700,
                        fontSize: 9,
                        letterSpacing: '0.1em',
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.background = 'rgba(253,245,232,0.22)';
                      e.currentTarget.style.color = 'rgba(253,245,232,0.9)';
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.background = 'rgba(253,245,232,0.12)';
                      e.currentTarget.style.color = 'rgba(253,245,232,0.6)';
                    }}
                  >
                    Copy
                  </button>
                ...
                >
                  Paste this code into any webpage. The survey runs in a clean, no-chrome embed
                  view.
                </p>
              </div>
            )}
            ...
            {tab === 'email' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                >
                  Send a personalised invitation with the survey link directly to someone&apos;s
                  inbox.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <input
                  ...
                  <motion.button
                    ...
                    onMouseEnter={(e) => {
                      if (!sending) e.currentTarget.style.background = 'var(--coral)';
                    }}
                    onMouseLeave={(e) => {
                      if (!sending) e.currentTarget.style.background = 'var(--espresso)';
                    }}
                  >
                    {sending ? '…' : 'Send'}
                  </motion.button>
