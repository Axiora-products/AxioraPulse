...
const OPERATORS = {
  ...
  multiple_choice: [
    ...
    { v: 'not_includes', l: 'does not include' },
  ],
  dropdown: [
    { v: 'equals', l: 'is' },
    { v: 'not_equals', l: 'is not' },
...
export default function ConditionalLogicEditor({ question, allQuestions, onChange }) {
  ...
  const eligible = allQuestions
    .slice(0, myIndex)
    .filter((q) => !['file_upload'].includes(q.question_type));
  ...
  function onOp(v) {
    setOperator(v);
    commit(sourceId, v, value);
  }
  function onVal(v) {
    setValue(v);
    commit(sourceId, operator, v);
  }
          ...
          >
            ...
            >
              ...
              {sourceId && value && (
                ...
                >
                  ✓ This question will only appear when{' '}
                  <strong style={{ fontWeight: 500 }}>
                    Q{eligible.findIndex((q) => (q._id || q.id) === sourceId) + 1}
                  </strong>{' '}
                  {ops.find((o) => o.v === operator)?.l}{' '}
                  <strong style={{ fontWeight: 500 }}>
