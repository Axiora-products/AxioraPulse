        ...
        try:
            ...
            if new_status == SurveyStatusEnum.active:
                ...
                if q_count < 2:
                    raise HTTPException(
                        status_code=400,
                        detail="At least 2 questions are required to publish",
                    )
...
def share_survey(
    ...
    target_user = (
        ...
        .filter(
            UserProfile.id == body.shared_with,
    ...
    existing = (
        ...
        .filter(
            SurveyShare.survey_id == survey_id,
            SurveyShare.shared_with == body.shared_with,
        )
        .first()
    )
    if existing:
