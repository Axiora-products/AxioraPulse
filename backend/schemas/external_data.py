...
class ObjectionLibraryInput(BaseModel):
    custom_objections: Optional[List[Dict[str, str]]] = Field(
        None,
        description="List of {objection: str, response: str} pairs"
    )
...
class FundraisingTimelineInput(BaseModel):
    ...
    key_milestones_for_close: Optional[List[str]] = Field(
        None,
        description="e.g. ['Reach 100 paying customers', '₹1L MRR', 'Hire CTO']"
    )
...
class CompetitiveMatrixInput(BaseModel):
    competitors: Optional[List[Dict[str, Any]]] = Field(
        None,
        description="List of {name, pricing, strengths, weaknesses, market_share}"
    )
...
class PartnershipInput(BaseModel):
    strategic_partners: Optional[List[Dict[str, str]]] = Field(
        None,
        description="List of {name, type, status} e.g. {name: 'Razorpay', type: 'Technology', status: 'Active'}"
    )
...
class BoardAdvisorInput(BaseModel):
    board_members: Optional[List[Dict[str, str]]] = Field(
        None,
        description="List of {name, background, equity_pct}"
    )
    advisors: Optional[List[Dict[str, str]]] = Field(
        None,
        description="List of {name, expertise, equity_pct}"
    )
...
class ExitStrategyInput(BaseModel):
    ...
    comparable_exits: Optional[List[Dict[str, str]]] = Field(
        None,
        description="e.g. [{company: 'Capillary Tech', exit_value: '$200M', year: '2023'}]"
    )
