...
def upgrade() -> None:
    ...
    def safe_drop_constraint(name, table, type_):
        ...
        exists = conn.execute(
            sa.text(
                "SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = :n AND table_name = :t"
            ),
            {"n": name, "t": table},
        ).fetchone()
        if exists:
    ...
    def safe_drop_index(name, table_name):
        conn = op.get_bind()
        exists = conn.execute(sa.text("SELECT 1 FROM pg_indexes WHERE indexname = :n"), {"n": name}).fetchone()
        if exists:
