...
class UserProfile(Base):
    ...
    )  # Axiora team members bypass payment gates
    ...
    surveys_created = relationship(
        "Survey", back_populates="creator", foreign_keys="Survey.created_by"
    )
...
class Survey(Base):
    ...
    creator = relationship(
        "UserProfile", back_populates="surveys_created", foreign_keys=[created_by]
    )
    ...
    feedbacks = relationship(
        "SurveyFeedback", back_populates="survey", cascade="all, delete-orphan"
    )
...
class SurveyResponse(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    survey_id = Column(
        UUID(as_uuid=True), ForeignKey("surveys.id", ondelete="CASCADE"), index=True, nullable=False
    )
...
class Payment(Base):
    ...
    plan_id = Column(
        UUID(as_uuid=True), ForeignKey("plans.id", ondelete="SET NULL"), index=True, nullable=True
    )
